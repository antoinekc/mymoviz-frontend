...
function Home() {
  ...
  const likedMoviesPopover = likedMovies.map((data, i) => {
    return (
      <div key={i} className={styles.likedMoviesContainer}>j
 ...
 // Hook d'effet
  ...
  const movies = moviesData.map((data, i) => {
    ...
    return <Movie key={i} updateLikedMovies={updateLikedMovies} isLiked={isLiked} title={data.title} overview={data.overview.length > 250 ? `${data.overview.substring(0, 250)}...` : data.overview} poster={`https://image.tmdb.org/t/p/w500${data.poster_path}`} voteAverage={data.voteAverage} voteCount={data.voteCount}/>;
  });
